data "google_compute_image" "cobaia_image" {
...
resource "google_compute_instance" "cobaia_instance" {
  name = format("%s-%s", var.project, var.env)
  machine_type = lookup(var.machine_size, var.env)
  zone = var.zone
  boot_disk {
    initialize_params {
      image = format("%s/%s",data.google_compute_image.cobaia_image.project , data.google_compute_image.cobaia_image.family)
    }
  }
  network_interface {
    network = "default"
...
resource "google_compute_firewall" "allow-cobaia" {
